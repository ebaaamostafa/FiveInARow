class Board:
    ...
    def __check_line(self, x: int, y: int, player: str, length : int) -> bool:
        ...
        if x + length - 1 < self.size and y + length - 1 < self.size and all(self.board[x + i][y + i] == player for i in range(length)):
            return True
        
        # Check diagonal (bottom-left to top-right)
        if x - length - 1 >= 0 and y + length - 1 < self.size and all(self.board[x - i][y + i] == player for i in range(length)):
            return True

        return False

    # for huristic evaluation
